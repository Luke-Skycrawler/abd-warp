...
from const_params import vec3, mat33, scalar
insert_vec2i = insert_overload(wp.vec2i)
mat32f = wp.types.matrix(shape = (3, 2), dtype = scalar)
...
@wp.kernel
def bvh_triangles(dialation: scalar, V: wp.array(dtype = vec3), F: wp.array2d(dtype = int), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    tid = wp.tid()
    ...
    uppers[tid] = wp.vec3f(vec3(ux, uy, uz) + vec3(dialation))
    lowers[tid] = wp.vec3f(vec3(lx, ly, lz) - vec3(dialation))

@wp.kernel
def bvh_edges(dialation: scalar, V: wp.array(dtype = vec3), E: wp.array2d(dtype = int), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    ...
    uppers[eid] = wp.vec3f(vec3(ux, uy, uz) + vec3(dialation))
    lowers[eid] = wp.vec3f(vec3(lx, ly, lz) - vec3(dialation))

@wp.kernel
def bvh_points(dialation: scalar, V: wp.array(dtype = vec3), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    pid = wp.tid()

    l = wp.vec3f(V[pid]) - wp.vec3f(wp.float32(dialation))
    u = wp.vec3f(V[pid]) + wp.vec3f(wp.float32(dialation))
...
class BvhBuilder:
    ...
    def build_point_bvh(self, V, dialation):
        uppers = wp.zeros(V.shape[0], dtype = wp.vec3f)
    ...
    def bulid_triangle_bvh(self, V, F, dialation):
        ...
        uppers = wp.zeros(F.shape[0], dtype = wp.vec3f)
        lowers = wp.zeros_like(uppers)
    ...
    def build_edge_bvh(self, V, E, dialation):
        uppers = wp.zeros(E.shape[0], dtype = wp.vec3f)
        lowers = wp.zeros_like(uppers)  
    ...
    def build_body_bvh(self, bodies: List[WarpMesh], dialation):
        ...
        _uppers = wp.from_numpy(np.array(uppers), dtype = wp.vec3f, shape = (len(uppers)))
        _lowers = wp.from_numpy(np.array(lowers), dtype = wp.vec3f, shape = (len(lowers)))
...
@wp.kernel
def bvh_triangle_trajectory(V: wp.array(dtype = vec3), F: wp.array2d(dtype = int), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    tid = wp.tid()
    ...
    us = vec3(ux, uy, uz)
    ls = vec3(lx, ly, lz)
    uppers[tid] = wp.max(uppers[tid], wp.vec3f(us))

    lowers[tid] = wp.min(lowers[tid], wp.vec3f(ls))


@wp.kernel
def bvh_edge_trajectory(V: wp.array(dtype = vec3), E: wp.array2d(dtype = int), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    ...
    uppers[eid] = wp.max(uppers[eid], wp.vec3f(vec3(ux, uy, uz)))
    lowers[eid] = wp.min(lowers[eid], wp.vec3f(vec3(lx, ly, lz)))

@wp.kernel
def bvh_point_trajectory(V: wp.array(dtype = vec3), uppers: wp.array(dtype = wp.vec3f), lowers: wp.array(dtype = wp.vec3f)):
    pid = wp.tid()
